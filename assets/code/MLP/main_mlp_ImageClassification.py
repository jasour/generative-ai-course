...
def train(model, input_size, batch_size, num_epochs, learning_rate):
    ...
    # MNIST training set has 60,000 samples
    # Composition of 2 transforms: 1) ToTensor: converts a PIL image or numpy array to a PyTorch tensor. 2) Normalize: normalizes the tensor with mean and std deviation of 0.5.
       # ToTensor: converts the image to a tensor of shape (1, 28, 28) and Normalization: scales the pixel values to be between -1 and 1.
       # Normalize: subtracts the mean (0.5) and divides by the standard deviation (0.5) for each channel.
    ...
    for epoch in range(num_epochs):
        for batch_idx, (images, labels) in enumerate(train_loader): #batch_idx will go from 0 to (number of batches - 1  = 937) 
            images = images.view(-1, input_size).to(device)  # Flatten and move to device, images: A tensor of shape [batch_size, 1, 28, 28]. Resulting shape: [batch_size, 784]
