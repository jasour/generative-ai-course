...
class TextDataset(Dataset): #randomly keep only max_samples samples.
    def __init__(self, dataset_name="ag_news", split="train", tokenizer=None, block_size=64, max_samples=10000):
        self.dataset = load_dataset(dataset_name, split=split)
        self.tokenizer = tokenizer or GPT2Tokenizer.from_pretrained("gpt2")
        self.block_size = block_size

        # ✅ Randomly select `max_samples` from full dataset
        if len(self.dataset) > max_samples:
            self.dataset = self.dataset.select(random.sample(range(len(self.dataset)), max_samples))

        self.inputs = []
        self.labels = []

        for i, item in enumerate(self.dataset['text']):  
            tokens = self.tokenizer.encode(item)
            for j in range(len(tokens) - block_size):
                self.inputs.append(tokens[j : j + block_size])
                self.labels.append(tokens[j + 1 : j + block_size + 1])
        ...
        print(f"✅ Final dataset size: {len(self.inputs)} training samples.")
...
if __name__ == "__main__":
    ...
    # ✅ Use AG News dataset instead of data.txt
    dataset = TextDataset(dataset_name="ag_news", tokenizer=tokenizer, block_size=config.block_size)
