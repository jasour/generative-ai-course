...
# non-saturating formulation GAN formulation
# we will train different variants of GANs on an easy 1D dataset.

# Here, we will use the non-saturating formulation of the GAN objective.
# We have two separate objectives:
#
#     L^(D) = E_{x ~ p_data} [ log(D(x)) ]
#             + E_{z ~ p_z} [ log(1 - D(G(z))) ]
#
#     L^(G) = - E_{z ~ p_z} [ log(D(G(z))) ]
...
class GANTrainer:
    ...
    def discriminator_loss(self, real_data, fake_data):
        """
        Discriminator objective (maximizing):
          L^(D) = E[log(D(x))] + E[log(1 - D(G(z)))]
        We return the negative so we can do gradient descent on it.
        """
    ...
    def generator_loss(self, fake_data):
        ...
        # - log(D(G(z)))
        loss_G = -torch.log(D_fake_prob + 1e-8).mean()
        return loss_G
    ...
    def train(self):
        ...
        for epoch in range(self.num_epochs):
            ...
            for batch_idx, batch in enumerate(self.dataloader):
                ...
                loss_D = self.discriminator_loss(batch_real, fake_data.detach())
                loss_D.backward()
